...
CreateThread(function()
    ...
    for k,_ in pairs(TokoCFG.Shops) do
	...
	swapHook = exports.ox_inventory:registerHook('swapItems', function(payload)
		for k,v in pairs(TokoCFG.Shops) do
			if payload.fromInventory == k then
				TriggerEvent('lynn_stockshop:refreshShop', k)
			elseif payload.toInventory == k and tonumber(payload.fromInventory) ~= nil then
				TriggerClientEvent('lynn_stockshop:setProductPrice', payload.fromInventory, k, payload.toSlot)
			end
		end
	end, {})
...
RegisterNetEvent('lynn_stockshop:refreshShop', function(shop)
	Wait(250)
	...
	exports.ox_inventory:RegisterShop(shop, {
		name = TokoCFG.Shops[shop].label,
		inventory = stashItems,
		locations = {
			TokoCFG.Shops[shop].locations.shop.coords,
		}
	})
end)

RegisterNetEvent('lynn_stockshop:setData', function(shop, slot, price)
	...
	TriggerEvent('lynn_stockshop:refreshShop', shop)
end)
